...
static void *find_next_fit(size_t size) {
    void *new_bp;
    for (new_bp = next_fit_p; GET_SIZE(HDRP(new_bp)) > 0; new_bp = NEXT_BLKP(new_bp)) {
        if (!GET_ALLOC(HDRP(new_bp)) && (size <= GET_SIZE(HDRP(new_bp)))) {
            next_fit_p = new_bp;
            return new_bp;
        }
    }
